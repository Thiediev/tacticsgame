...
public class NWTile : MonoBehaviour
{
    ...
    public List<NWTile> neighbors = new List<NWTile>();
    ...
    void generateNeighbors()
    {
        ...
        if (gridPosition.y > 0)
        {
            ...
            neighbors.Add(NWGameManager.instance.map[(int)Mathf.Round(n.x)][(int)Mathf.Round(n.y)]);
        }



        //down
        if (gridPosition.y < NWGameManager.instance.map.Count - 1)
        {
            Vector2 n = new Vector2(gridPosition.x, gridPosition.y + 1);
            neighbors.Add(NWGameManager.instance.map[(int)Mathf.Round(n.x)][(int)Mathf.Round(n.y)]);
        }
        ...
        //right
        if (gridPosition.x < NWGameManager.instance.map.Count - 1)
        {
            Vector2 n = new Vector2(gridPosition.x + 1, gridPosition.y);
            neighbors.Add(NWGameManager.instance.map[(int)Mathf.Round(n.x)][(int)Mathf.Round(n.y)]);
        }
    ...
    public void OnGUI()
    {
        if (Application.loadedLevelName == "NetworkVersus")
        {
            ...
            if (buttonPressed && myTile != null)
            {
                ...
                if (myTile.canBuildLandUnits && ((myTile.addsIncomePlayerOne && NWGameManager.instance.playerOneTurn) || (myTile.addsIncomePlayerTwo && NWGameManager.instance.playerTwoTurn)))
                {
                    ...
                    if (readyToBuild)
                    {
                        ...
                        NWGameManager.instance.BuildUnitsGUI();
                    }
